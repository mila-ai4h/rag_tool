...
def delete_collection(name: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to delete collection '{name}': {response.text}")
            return False
...
def create_collection(name: str) -> bool:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to create collection '{name}': {response.text}")
            return False
    except Exception as e:
...
def index_pdf(collection: str, file_path: str,
              source_id: str, tags: str, extras: str) -> bool:
    ...
    try:
        ...
        with open(file_path, 'rb') as f:
            ...
            if response.status_code == 200:
                ...
                logger.error(
                    f"Failed to index PDF {file_path}: {response.text}")
                return False
    except Exception as e:
...
def index_url(collection: str, url: str, source_id: str,
              tags: str, extras: str) -> bool:
    ...
    try:
        with open(csv_path, 'r') as f:
            ...
            for row_num, row in enumerate(
                    reader, start=2):  # start=2 because row 1 is header
                ...
                if doc_type not in ['pdf', 'url']:
                    logger.error(
                        f"Line {row_num}: Invalid document type: {doc_type}")
                    error_count += 1
                    continue
...
def main():
    ...
    parser.add_argument(
        'csv_path',
        help='Path to the CSV file containing document information')
    parser.add_argument(
        '--api-key',
